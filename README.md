...
Расширение для Chrome: чат по контенту страниц с **локальной** LLM (LM Studio, Ollama и др.). Модель может **просматривать текущую вкладку** и выдавать по ней краткое саммари; при включённых **MCP**-инструментах — вызывать их прямо из чата. Данные не уходят в облако — только ваш LLM endpoint.
...
| **Хранение** | IndexedDB: история чата. `chrome.storage.sync` / `local`: LLM, MCP, лимиты контекста, инструкции агента. Локально в браузере. |
| **Интерфейс** | Side panel (Chrome) или popup (Edge, Yandex). Многоязычность (EN/RU). Настройки: LLM, контекст чата и rolling-summary, браузерная автоматизация, оркестратор, MCP, текстовые инструкции для агента; полная форма — на странице **Options** расширения. См. [docs/SETTINGS.md](docs/SETTINGS.md). |
...
3. **Настройте LLM**: во вкладке **Settings** панели/popup выберите профиль и max tokens; чтобы добавить endpoint и модель — **Add…** откроет страницу **Options** (или: меню расширения → Параметры). Пример URL: `http://localhost:1234` (LM Studio, тип «Chat») или свой OpenAI-совместимый endpoint.
4. (Опционально) **MCP** и **Instructions** — в тех же настройках или на странице Options; подробнее [docs/SETTINGS.md](docs/SETTINGS.md).
5. В чате задайте вопрос: например, *«О чём эта страница?»* или *«Кратко перескажи содержимое»* — модель получит контент текущей вкладки и выдаст саммари. Остальные вопросы обрабатываются как обычный чат с LLM (с вызовом MCP-инструментов, если они включены).
