...
| **Интерфейс** | Side panel (Chrome) или popup (Edge, Yandex). Многоязычность (EN/RU). Настройки LLM (endpoint, модель) и опционально MCP (JSON конфиг серверов); при использовании Confluence — URL и API Token. |
...
1. Откройте любую веб-страницу (или Confluence, если используете).
...
5. (Опционально) **Confluence**: если нужен поиск по Confluence — укажите Base URL и API Token в настройках.
...
| `src/api/confluence.ts` | Опционально: REST API Confluence (страницы, spaces) |
