...
Расширение для Chrome: чат по контенту страниц с **локальной** LLM (LM Studio, Ollama и др.). Модель может **просматривать текущую вкладку** и выдавать по ней краткое саммари; при включённых **MCP**-инструментах — вызывать их прямо из чата. Данные не уходят в облако — только ваш LLM endpoint (и при необходимости ваш wiki).
...
5. (Опционально) **Wiki**: если нужен поиск по wiki — укажите Base URL и API Token в настройках.
...
| `src/api/confluence.ts` | Опционально: wiki REST API (страницы, spaces) |
