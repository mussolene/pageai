...
2. **Состояние и история:** хранить в `chrome.storage` / IndexedDB, не в глобальных переменных background — они теряются при остановке worker’а.
3. **Уведомление при закрытом popup:** если стрим идёт в background и popup закрыли — не прерывать стрим по `onDisconnect`; по завершении сохранять ответ в storage и показывать уведомление (см. текущую реализацию в background).
4. **Тестирование:** проверять длинные запросы (thinking-модели, медленный ответ) в Chrome с открытой панелью и с закрытым popup.
