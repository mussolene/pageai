...
| Высокий | Секреты (API-ключи, токены Confluence, MCP headers) в `chrome.storage.sync` без шифрования |
| Высокий | LLM endpoint не валидируется — возможна отправка данных на произвольный URL без предупреждения |
| Средний | CQL в Confluence: поисковые термины и spaceKey без экранирования кавычек |
...
| 2 | Хранение секретов | llm/client.ts, options.ts, api/confluence.ts, AGENTS.md | Сделано | Секреты в storage.local |
| 3 | Валидация LLM endpoint | llm/client.ts, options.ts | Сделано | isLocalLlmEndpoint, предупреждение в UI |
| 4 | Экранирование CQL | src/api/confluence.ts, tests | Сделано | escapeCqlString в buildCQL |
| 5 | CSP в manifest | manifest.json | Сделано | extension_pages |
| 6 | Политика конфиденциальности | PRIVACY.md | Сделано | — |
| 7 | Обновление документации | CODE_REVIEW.md, TECH_DEBT.md | Сделано | — |
...
**Файлы:** `src/llm/client.ts` (getLlmConfig), `src/ui/options.ts`, `src/api/confluence.ts` (getConfig), `AGENTS.md`.

- Секреты хранить в `chrome.storage.local`: `llmApiKey`, `confluenceApiToken`, `confluenceUsername`. MCP config с headers — либо вынести headers в local, либо оставить в sync с явным предупреждением в документации.
- При сохранении настроек в options.ts писать секреты в `chrome.storage.local`; при чтении в llm/client.ts и api/confluence.ts — читать из local (при отсутствии не подставлять из sync).
...
### Задача 4: Экранирование CQL в Confluence (средний)

**Файл:** `src/api/confluence.ts`, функция `buildCQL` (строки 211–224).

- `escapeCqlString(s: string): string` — экранировать `\` → `\\`, `"` → `\"`.
- В `buildCQL` использовать для каждого термина и каждого spaceKey.
- **DoD:** Unit-тест с запросом, содержащим `"` и `\`; переданный CQL корректен (можно мокать fetch и проверять URL/cql параметр).

### Задача 5: Content-Security-Policy (средний)
...
### Задача 6: Политика конфиденциальности (низкий)
