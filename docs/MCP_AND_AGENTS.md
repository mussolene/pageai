...
1. **Агенты (без MCP)**  
   - Убедиться, что в Cursor подхватываются workspace rules: открыть чат и проверить, что ответы учитывают AGENTS.md/agent.md (упоминание Local-first, BYOM, Confluence).
...
2. **MCP в Cursor**  
   ...
   - Проверка: в чате Cursor вызвать инструмент, который предоставляет MCP-сервер (если такие есть в UI).
...
- **Реализовано полностью (интеграция агента с MCP):**
  ...
  - Цикл агента в background: при отправке сообщения в чат (и в потоковом, и в обычном режиме) сначала вызывается `getEnabledMcpToolsWithMap()`. Если есть включённые MCP-инструменты, выполняется `runAgentWithMcpTools(userMessage, systemPrompt)`: в цикле (до 5 итераций) вызывается `chatWithLLMOneRound` с инструментами; при ответе с `tool_calls` для каждого вызова выполняется `callMcpTool`, результаты добавляются в диалог как сообщения с ролью `tool`, затем снова запрос к LLM; при ответе с текстом — возврат пользователю.

**Итог:** агент (LLM) **может** вызывать подключённые MCP-инструменты из чата расширения, если в настройках включён хотя бы один MCP-сервер и модель поддерживает function calling (tool_calls).
...
- **MCP в расширении**: только настройка и список инструментов; вызов инструментов агентом (LLM) из чата расширения не реализован.
