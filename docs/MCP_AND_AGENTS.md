...
### Внутренний провайдер «Browser» (браузерная автоматизация)

Расширение предоставляет агенту **внутренний** набор инструментов для работы со страницей (без отдельного MCP-сервера). При включённой опции «Browser automation» в настройках в системный промпт добавляется блок `[BROWSER TOOLS]` и следующие инструменты (формат OpenAI function calling):

| Инструмент     | Описание |
|----------------|----------|
| **page_read**  | Прочитать текущую страницу: текст и метаданные; по умолчанию — краткое саммари, опционально `mode: "full"` для полного текста. |
| **page_click** | Клик по элементу по видимому тексту или CSS-селектору (кнопки, ссылки, вкладки). |
| **page_fill**  | Заполнить поле ввода по метке/placeholder/name или селектору (поиск, комментарий, формы). |
| **page_navigate** | Перейти по URL в текущей вкладке. |
...
При сборке системного промпта background подставляет эти блоки в порядке: базовый промпт → `[RULES]` → `[SKILLS]` → `[BROWSER TOOLS]` / `[MCP TOOLS]` → `[INTENT]`. Агент получает единый контекст: правила, навыки и список инструментов.

### Фоновая работа и мультиагентность

- **Фон:** запросы к агенту выполняются в service worker. Если панель или popup закрыты во время выполнения, порт отключается, но расчёт продолжается; по завершении ответ сохраняется в историю чата (IndexedDB) и показывается уведомление «Chat ready».
- **Очередь задач:** входящие стрим-запросы чата (`CHAT_STREAM_REQUEST`) попадают в очередь. Одновременно выполняется не более **2** задач (`MAX_CONCURRENT_CHAT_TASKS`). Остальные ждут в очереди и запускаются по мере освобождения слотов. Реализация: `chatTaskQueue`, `enqueueChatStreamTask`, `runOneChatStreamTask` в `src/background/index.ts`.
